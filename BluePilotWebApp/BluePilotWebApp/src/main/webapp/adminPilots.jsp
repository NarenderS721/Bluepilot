	...
	<header>
  	...
  	<section>
  		...
  		<%
  			...
  			ArrayList<HashMap> pilots=d.getAllPilots();
  			d.disconnect();
  			for(HashMap pilot:pilots){
  		%>
  		<div class="p-2 m-2 bg-light">
  			<p>
  			<img src="GetPhoto?email=<%=pilot.get("email") %>&userType=pilot" height="40" style="border-radius:50%;" /> 
  			<b><%=pilot.get("name") %></b> , 
  			Gender: <%=pilot.get("gender") %> 
  			Phone: <%=pilot.get("phone") %> ,
  			Experience: <%=pilot.get("experience") %> ,
  			Address: <%=pilot.get("address") %> ,
  			[<%=pilot.get("status") %>]
  			<%
  				if(pilot.get("status").equals("pending")){
	  			%>		
	  				<a class="btn btn-success"  href="PilotStatusUpdate?email=<%=pilot.get("email") %>&status=accepted">Accept</a>
	  				<a class="btn btn-danger"  href="PilotStatusUpdate?email=<%=pilot.get("email") %>&status=rejected">Reject</a>
	  			<% 
  				}
  			%>
  			</p>
